...
// A node stand-in: carries the key a lookup matches on, plus the intrusive link.
struct FakeDevice {
  explicit FakeDevice(uint16_t id) :
    clientCanId(id) {}

  [[nodiscard]] uint16_t getClientCanId() const { return clientCanId; }
  [[nodiscard]] FakeDevice* getNext() const { return next; }
  void setNext(FakeDevice* device) { next = device; }
...
bool test_an_empty_list_routes_nowhere() {
  ...
  IntrusiveList<FakeDevice> devices;
  IS_TRUE(devices.first() == nullptr);
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x101U; }) == nullptr);
  END_IT
}
...
bool test_a_registered_device_is_found_by_its_id() {
  ...
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x101U; }) == &device);
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x102U; }) == nullptr);       // an unknown sender routes nowhere
  END_IT
}
...
bool test_every_device_is_found_by_its_own_id() {
  ...
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x101U; }) == &first);
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x102U; }) == &second);
  IS_TRUE(devices.findIf([](const FakeDevice* d) -> bool { return d->getClientCanId() == 0x103U; }) == &third);
  END_IT
}
