...
private:
  ...
  enum class DataTransferError : DataTransferErrorType {
    // clang-format off
    NONE                      = 0U,                     // No error.
    FILE_SIZE_ZERO            = 1UL << 0U,              // The provided file size is zero.
    FILE_MD5_NULLPTR          = 1UL << 1U,              // The MD5 pointer is null.
    FILE_MD5_INVALID          = 1UL << 2U,              // The MD5 string is invalid (e.g., empty).
    FILE_NAME_NULLPTR         = 1UL << 3U,              // The file name pointer is null.
    FILE_NAME_INVALID         = 1UL << 4U,              // The file name is invalid (e.g., empty).
    FILE_NAME_NOT_ALLOWED     = 1UL << 5U,              // The file name is not in the list of accepted locations.
    NOT_ENOUGH_STORAGE        = 1UL << 6U,              // Not enough storage space to store the file.
    BEGIN_NOT_CALLED          = 1UL << 7U,              // The begin() method was not called prior to storing file pieces.
    WRONG_FILE_PIECE_NUMBER   = 1UL << 8U,              // The file piece number is not sequential.
    FILE_ALREADY_STORED       = 1UL << 9U,              // The file has already been fully stored.
    FILE_DATA_NULLPTR         = 1UL << 10U,             // The provided file data pointer is null.
    B64_FILE_DATA_SIZE_ERROR  = 1UL << 11U,             // Base64 file data size is not valid.
    FILE_PIECE_SIZE_OVEFLOW   = 1UL << 12U,             // The decoded file piece size exceeds the maximum allowed.
    FILE_PIECE_SIZE_ERROR     = 1UL << 13U,             // The file piece size is incorrect.
    B64_DECODED_SIZE_ERROR    = 1UL << 14U,             // Error in the size after base64 decoding.
    TEMP_FILE_OPENING_ERROR   = 1UL << 15U,             // Error opening the temporary file for writing.
    TEMP_FILE_WRITING_ERROR   = 1UL << 16U,             // Error writing to the temporary file.
    RECEIVED_FILE_SIZE_ERROR  = 1UL << 17U,             // The received file size does not match the expected size.
    FILE_MD5_ERROR            = 1UL << 18U,             // MD5 checksum validation failed.
    TEMP_FILE_RENAMING_ERROR  = 1UL << 19U,             // Error renaming the temporary file.
    FW_UPGRADE_BEGIN_FAILED   = 1UL << 20U,             // Firmware upgrade initialization failed.
