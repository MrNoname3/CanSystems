...
private:
  ...
  static constexpr const char PROGMEM mqttDiagPayload[] = R"({"cause":"%s","at":"%s","downSec":%u,"n":%u,"pingRetry":%hu})"; // Disconnect diagnostics JSON (published to the "diag" subtopic via publishRetained); args: cause string, drop ISO UTC time, offline seconds, reconnect counter, refused keep-alive pings.
  // clang-format on
  ...
  static constexpr uint8_t diagPayloadBufSize = 128U;                                                     // 50 fixed chars + cause (28: "MQTT_CONNECT_BAD_CREDENTIALS") + ISO time (20) + 2x uint32 (10 each) + uint16 (5) + null.
